...
__device__ int check_rect_cross_multi_heads(const Point &p1, const Point &p2, const Point &q1, const Point &q2){
...
__device__ inline int intersection(const Point &p1, const Point &p0, const Point &q1, const Point &q0, Point &ans){
    // fast exclusion
    if (check_rect_cross_multi_heads(p0, p1, q0, q1) == 0) return 0;
...
__global__ void boxes_overlap_kernel_multi_heads(const int num_a, const float *boxes_a, const int num_b, const float *boxes_b, float *ans_overlap){
...
__global__ void boxes_iou_bev_kernel_multi_heads(const int num_a, const float *boxes_a, const int num_b, const float *boxes_b, float *ans_iou){
    // params boxes_a: (N, 7) [x, y, z, dx, dy, dz, heading]
    // params boxes_b: (M, 7) [x, y, z, dx, dy, dz, heading]
    const int a_idx = blockIdx.y * THREADS_PER_BLOCK + threadIdx.y;
    const int b_idx = blockIdx.x * THREADS_PER_BLOCK + threadIdx.x;
...
__global__ void nms_kernel_multi_heads(const int boxes_num, const float nms_overlap_thresh,
...
__global__ void nms_multi_heads_kernel(const int boxes_num, const float nms_overlap_thresh,
                                  const float *boxes, unsigned long long *mask){
    //params: boxes (N, 7) [x, y, z, dx, dy, dz, heading]
    //params: mask (N, N/THREADS_PER_BLOCK_NMS)
...
NmsMultiHeadsCuda::NmsMultiHeadsCuda(const int num_threads, const int num_box_corners,
                 const float nms_overlap_threshold)
        : num_threads_(num_threads),
          num_box_corners_(num_box_corners),
          nms_overlap_threshold_(nms_overlap_threshold) {}

void NmsMultiHeadsCuda::DoNmsCuda(const int host_filter_count,
    ...
    nms_kernel_multi_heads<<<blocks, threads>>>(host_filter_count, nms_overlap_threshold_, dev_sorted_box_for_nms, dev_mask);
