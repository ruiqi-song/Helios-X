...
int NMSCenter::nms_gpu(const float* boxes_data, long* keep_data, int boxes_num,  float nms_overlap_thresh){
...
int NMSCenter::raw_nms_gpu(const float* reg,  const float* height, const float* dim , const float* rot, const int* indexs, long* keep_data, int boxes_num,  float nms_overlap_thresh){
    // params boxes: (N, 7) [x, y, z, dx, dy, dz, heading]
    // params keep: (N)
