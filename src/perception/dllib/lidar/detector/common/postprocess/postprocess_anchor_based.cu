...
#include "postprocess_anchor_based.h"
...
__global__ void filter_kernel(const float* box_preds, const float* cls_preds, const float* dir_preds, const int* anchor_mask,
                              ...
                              float* filtered_box, float* filtered_score,int* filtered_cls, int* filtered_dir, float* box_for_nms, int* filter_count,
                              const float FLOAT_MIN, const float FLOAT_MAX, const float score_threshold,
                              const int NUM_BOX_CORNERS, const int NUM_OUTPUT_BOX_FEATURE, const int num_classes)
{
  ...
  for (int i = 1; i < num_classes; i++) {
      float cls_score = 1 / (1 + expf(- cls_preds[tid*num_classes + i - 1]));
      if (cls_score > score) {
          score = cls_score;
          cls_id = i;
      }
  ...
  if(anchor_mask[tid] == 1 && score > score_threshold)
  {
    ...
    filtered_cls[counter] = cls_id;
    //filtered_score[counter + 1] = cls_id;
...
void PostprocessCuda::doPostprocessCuda(const float* rpn_box_output, const float* rpn_cls_output, const float* rpn_dir_output,
        ...
        const float* dev_anchors_dx, const float* dev_anchors_dy, const float* dev_anchors_dz, const float* dev_anchors_ro,
        float* dev_filtered_box, float* dev_filtered_score, int* dev_filtered_cls, int* dev_filtered_dir, float* dev_box_for_nms, int* dev_filter_count,
        const int num_classes, std::vector<float>& out_detection)
        //std::vector<Box3D>& output_boxes)
{

    // decoding output boxes and convert boxes to BEV for nms
    filter_kernel<<<NUM_ANCHOR_X_INDS_*NUM_ANCHOR_R_INDS_, NUM_ANCHOR_Y_INDS_>>>
                                ...
                                dev_filtered_box, dev_filtered_score, dev_filtered_cls, dev_filtered_dir, dev_box_for_nms, dev_filter_count,
                                FLOAT_MIN_, FLOAT_MAX_, score_threshold_,
                                NUM_BOX_CORNERS_, NUM_OUTPUT_BOX_FEATURE_, num_classes);

    // init the number of filtered output
    ...
    //sort BEV boxes by indexes kernel
    int* dev_indexes;
...
//        Box3D box;
//
//        box.x = host_filtered_box[keep_inds[i]*NUM_OUTPUT_BOX_FEATURE_+0];
//        box.y = host_filtered_box[keep_inds[i]*NUM_OUTPUT_BOX_FEATURE_+1];
//        box.z = host_filtered_box[keep_inds[i]*NUM_OUTPUT_BOX_FEATURE_+2];
//        box.h = host_filtered_box[keep_inds[i]*NUM_OUTPUT_BOX_FEATURE_+3];
//        box.w = host_filtered_box[keep_inds[i]*NUM_OUTPUT_BOX_FEATURE_+4];
//        box.l = host_filtered_box[keep_inds[i]*NUM_OUTPUT_BOX_FEATURE_+5];
//        box.theta = host_filtered_box[keep_inds[i]*NUM_OUTPUT_BOX_FEATURE_+6];
//        box.cls = host_filtered_box[keep_inds[i]*NUM_OUTPUT_BOX_FEATURE_+7];
