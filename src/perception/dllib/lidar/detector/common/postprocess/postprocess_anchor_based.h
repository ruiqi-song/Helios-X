...
#include "lidar/detector/common/nms/3d_nms.h"
//#include "base/lidar_base_detector.h"
...
public:
  ...
  */
  PostprocessCuda(const float FLOAT_MIN, const float FLOAT_MAX, const int NUM_ANCHOR_X_INDS,
                  ...
                  const int NUM_OUTPUT_BOX_FEATURE, const int num_classes);
  ...
  */
  void doPostprocessCuda(const float* rpn_box_output, const float* rpn_cls_output, const float* rpn_dir_output,
                         ...
                         float* dev_filtered_score, int* dev_filtered_cls, int* dev_filtered_dir, float* dev_box_for_nms,
                         int* dev_filter_count, const int num_classes, std::vector<float>& out_detection);
