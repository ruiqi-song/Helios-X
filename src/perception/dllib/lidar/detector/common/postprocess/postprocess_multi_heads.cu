...
#include "postprocess_multi_heads.h"
#include <stdio.h>
...
PostprocessMultiHeadsCuda::PostprocessMultiHeadsCuda(const int num_threads, const float float_min, const float float_max,
    ...
    nms_cuda_ptr_.reset(
            new NmsMultiHeadsCuda(num_threads_, num_box_corners_, nms_overlap_threshold_));

}


void PostprocessMultiHeadsCuda::DoPostprocessCuda(
        float* cls_pred_0,
        float* cls_pred_123,
        float* cls_pred_4,
        float* cls_pred_5,
...
//    GPU_CHECK(cudaMemcpy(&host_score[14 * num_anchor_per_cls_], cls_pred_89, num_anchor_per_cls_ * 2 * 2 * sizeof(float), cudaMemcpyDeviceToHost));
    ...
    GPU_CHECK(cudaMemcpy(&host_score[0 * num_anchor_per_cls_], cls_pred_0, num_anchor_per_cls_ * sizeof(float), cudaMemcpyDeviceToHost));
    GPU_CHECK(cudaMemcpy(&host_score[1 * num_anchor_per_cls_], cls_pred_123, num_anchor_per_cls_ * 3 * 3 * sizeof(float), cudaMemcpyDeviceToHost));
    GPU_CHECK(cudaMemcpy(&host_score[10 * num_anchor_per_cls_], cls_pred_4, num_anchor_per_cls_ *  sizeof(float), cudaMemcpyDeviceToHost));
    GPU_CHECK(cudaMemcpy(&host_score[11 * num_anchor_per_cls_], cls_pred_5, num_anchor_per_cls_ * sizeof(float), cudaMemcpyDeviceToHost));

    int stride[6] = {0 , 1 , 1 , 1 , 10 , 11 };
    int offset[6] = {0 , 0 , 1 , 2 , 0 , 0};
    ...
    for (int class_idx = 0; class_idx < num_class_; ++ class_idx) {  // hardcode for class_map as {0, 12 , 34 , 5 ,67 ,89}
        ...
        for (size_t anchor_idx = 0 ; anchor_idx < num_anchor_per_cls_ ; anchor_idx++)
        {
            ...
            float score_upper = 0;
            float score_lower = 0;
            float score_middle = 0;
            if (class_idx == 0 || class_idx == 5 || class_idx == 4) {
                score_upper =  1 / (1 + expf(-host_score[ stride[class_idx] * num_anchor_per_cls_ + anchor_idx ])); // sigmoid function

            }
            else {
                score_upper =  1 / (1 + expf(-host_score[ stride[class_idx] * num_anchor_per_cls_  + anchor_idx * 3  + offset[class_idx]]));
                score_middle =  1 / (1 + expf(-host_score[ stride[class_idx] * num_anchor_per_cls_  + num_anchor_per_cls_ * 3 + anchor_idx * 3  + offset[class_idx]]));
                score_lower =  1 / (1 + expf(-host_score[ stride[class_idx] * num_anchor_per_cls_  + num_anchor_per_cls_ * 6  + anchor_idx * 3 + offset[class_idx]]));
                // printf("up , low : %f ,%f \n", score_upper , score_lower);
            }

            score_vec.emplace_back(score_upper);
            ...
            //score_upper =  host_score[ stride[class_idx] * num_anchor_per_cls_  + anchor_idx * 2  + offset[class_idx]];
            //score_lower =  host_score[ stride[class_idx] * num_anchor_per_cls_  + (num_anchor_per_cls_ + anchor_idx) * 2 + offset[class_idx]];
            ...
            if (score_middle > score_threshold_ && host_filtered_count[class_idx] < nms_pre_maxsize_)  // filter out boxes which threshold less than score_threshold
            {
                host_filtered_score[host_filtered_count[class_idx]] = score_middle;
                for (size_t dim_idx = 0 ; dim_idx < 7 ; dim_idx++) // dim_idx = {x,y,z,dx,dy,dz,yaw}
                {
                    host_filtered_box[host_filtered_count[class_idx] * 7 + dim_idx] \
                    =  host_box[ class_idx * num_anchor_per_cls_ * num_output_box_feature_ + anchor_idx * num_output_box_feature_ + dim_idx];
    ...
    //std::cout << "score_vec[0]: " << score_vec[0] << std::endl;
