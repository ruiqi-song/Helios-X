...
#include "postprocess_center_based.h"



inline void RotateAroundCenter(Box3D& box, float (&corner)[4][2], float& cosVal, float& sinVal, float (&cornerANew)[4][2]){
...
inline float IoUBev(Box3D& boxA, Box3D& boxB){
...
void PostProcessAnchorFree::AlignedNMSBev(std::vector<Box3D>& predBoxs){

    if(predBoxs.size() == 0)
        return;

    std::sort(predBoxs.begin(),predBoxs.end(),[ ](Box3D& box1, Box3D& box2){return box1.score > box2.score;});
...
PostProcessAnchorFree::PostProcessAnchorFree() {
    Init();
}
...
void PostProcessAnchorFree::Init() {
    nms_.reset(new NMSCenter());
}

//void PostProcessAnchorFree::postprocess(const int &buffers, std::vector<Box> &predResult) {}

//void PostProcessAnchorFree::postprocess(const samplesCommon::BufferManager& buffers, std::vector<Box>& predResult){
//
//
...
void PostProcessAnchorFree::postprocessGPU(
        float* reg,
        float* height,
        float* rot,
        float* dim,
...
//    int32_t* cls = cls_;
    ...
    int boxSize = nms_->findValidScoreNum( score, SCORE_THREAHOLD, OUTPUT_H , OUTPUT_W);
    std::cout << " Num boxes before " << boxSize <<"\n";

    nms_->sort_by_key(score, dev_score_indexs, OUTPUT_W * OUTPUT_H);



    boxSize = boxSize > INPUT_NMS_MAX_SIZE ? INPUT_NMS_MAX_SIZE : boxSize;

    int boxSizeAft = nms_->rawNmsGpu(reg,  height, dim , rot, dev_score_indexs,
    ...
    nms_->gather_all(host_boxes, host_label,
    ...
    std::cout << "boxSizeAft:  " << boxSizeAft << std::endl;
