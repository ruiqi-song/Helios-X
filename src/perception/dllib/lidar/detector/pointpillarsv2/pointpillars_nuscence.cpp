...
#include "pointpillars_nuscence.h"

#include <chrono>
#include <iostream>
#include <iostream>

void PointPillarsNuScene::Init(YAML::Node& config)
{
    score_threshold_ = 0.1;
    nms_overlap_threshold_ = 0.2;
    use_onnx_ = true;
    pfe_file_ = "/home/ricky/waytous_server/src/perception/conf/dllib/lidar/detector/pointpillars/pointpillars_pfe_multiheads.onnx";
    backbone_file_ = "/home/ricky/waytous_server/src/perception/conf/dllib/lidar/detector/pointpillars/pointpillars_rpn_multiheads.onnx";
    pp_config_ = "/home/ricky/waytous_server/src/perception/conf/dllib/lidar/detector/pointpillars/pp_multihead.yaml";
...
PointPillarsNuScene::PointPillarsNuScene(){}


void PointPillarsNuScene::DeviceMemoryMalloc() {
    // for pillars
...
void PointPillarsNuScene::SetDeviceMemoryToZero() {
...
void PointPillarsNuScene::InitTRT(const bool use_onnx) {
    if (use_onnx_) {
...
void PointPillarsNuScene::OnnxToTRTModel(
...
void PointPillarsNuScene::EngineToTRTModel(
...
void PointPillarsNuScene::Detect(const float *in_points_array, const int in_num_points,
                                 std::vector<float> &out_detections, std::vector<Box3D> &predResult)
{
    ...
    gLogInfo << "output 3d bounding boxes is : " << out_boxes.size() / 7 << std::endl;
