...
#include "pointpillars_multi_heads.h"

#include <chrono>
#include <iostream>
#include <iostream>

void PointPillarsMultiHeads::Init(YAML::Node& config)
{
    score_threshold_ = 0.8;
    nms_overlap_threshold_ = 0.2;
    use_onnx_ = false;
    pfe_file_ = "/home/ricky/waytous_server/src/perception/conf/dllib/lidar/detector/pointpillars/pp_multihead_pfe_v2.onnx";
    backbone_file_ = "/home/ricky/waytous_server/src/perception/conf/dllib/lidar/detector/pointpillars/pp_mul_backbone_v3.onnx";
    pfe_trt_file_ = "/home/ricky/waytous_server/src/perception/conf/dllib/lidar/detector/pointpillars/pfe.trt";
    backbone_trt_file_ = "/home/ricky/waytous_server/src/perception/conf/dllib/lidar/detector/pointpillars/backbone.trt";
    pp_config_ = "/home/ricky/waytous_server/src/perception/conf/dllib/lidar/detector/pointpillars/pointpillar_ouster_mul_6class.yaml";
    ...
    kNumAnchorPerCls = kNumAnchorXinds * kNumAnchorYinds * 2; //* 2; //H * W * Ro = 32768
    kRpnBoxOutputSize = kNumAnchor * kNumOutputBoxFeature;
    kRpnClsOutputSize = kNumAnchor * kNumClass;
    kRpnDirOutputSize = kNumAnchor * 2;


    gLogInfo << "kGridXSize: " << kGridXSize << std::endl;
    gLogInfo << "kGridYSize: " << kGridYSize << std::endl;
    gLogInfo << "kNumAnchorXinds: " << kNumAnchorXinds << std::endl;
    gLogInfo << "kNumAnchorYinds: " << kNumAnchorYinds << std::endl;
    gLogInfo << "kNumAnchorPerCls: " << kNumAnchorPerCls << std::endl; //115200
    gLogInfo << "kAnchorStrides: " << kAnchorStrides << std::endl;
    gLogInfo << "kNumAnchor: " << kNumAnchor << std::endl;
    gLogInfo << "kRpnBoxOutputSize: " << kRpnBoxOutputSize << std::endl;
    gLogInfo << "kRpnBoxOutputSize: " << kRpnBoxOutputSize << std::endl;
    gLogInfo << "kRpnDirOutputSize: " << kRpnDirOutputSize << std::endl;
    gLogInfo << "kRpnInputSize: " << kRpnInputSize << std::endl;
    gLogInfo << "kpfeInputSize: " <<  kMaxNumPillars * kMaxNumPointsPerPillar *
                                      kNumGatherPointFeature << std::endl;

    gLogInfo << "kpfeOutputSize: " <<  kMaxNumPillars * 64 << std::endl;
    gLogInfo << "kNumIndsForScan: " <<  kNumIndsForScan  << std::endl;
    ...
    preprocess_points_cuda_ptr_.reset(new PreprocessPointsMultiHeadsCuda(
            kNumThreads,
            kNumPointFeature,
            kNumIndsForScan,
            kMaxNumPillars,
            kMaxNumPointsPerPillar, //32
...
//            kMinXRange, kMinYRange, kMinZRange));
//
    scatter_cuda_ptr_.reset(new ScatterMultiHeadsCuda(kNumThreads, kGridXSize, kGridYSize));

    const float float_min = std::numeric_limits<float>::lowest();
    const float float_max = std::numeric_limits<float>::max();
    postprocess_cuda_ptr_.reset(
            new PostprocessMultiHeadsCuda(kNumThreads,
                                          float_min, float_max,
...
PointPillarsMultiHeads::PointPillarsMultiHeads() {}



void PointPillarsMultiHeads::DeviceMemoryMalloc() {
    // for pillars
    ...
    GPU_CHECK(cudaMalloc(&rpn_buffers_[2],  kNumAnchorPerCls  * 3 * 3 * sizeof(float)));
...
//    GPU_CHECK(cudaMalloc(&rpn_buffers_[7],  kNumAnchorPerCls * kNumClass * kNumOutputBoxFeature * sizeof(float))); //boxes
    ...
    host_score_ =  new float[kNumAnchorPerCls * 12]();
...
void PointPillarsMultiHeads::SetDeviceMemoryToZero() {
...
void PointPillarsMultiHeads::InitTRT(const bool use_onnx) {
    if (use_onnx_) {
        ...
        EngineToTRTModel(pfe_trt_file_, &pfe_engine_);
        EngineToTRTModel(backbone_trt_file_, &backbone_engine_);
    }
...
void PointPillarsMultiHeads::OnnxToTRTModel(
    ...
    FILE* f = fopen("/home/ricky/pfe.trt", "wb");
...
void PointPillarsMultiHeads::EngineToTRTModel(
...
void PointPillarsMultiHeads::Detect(const float *in_points_array, const int in_num_points,
                                    std::vector<float> &out_detections, std::vector<Box3D> &predResult)
{
...
//    for(int i = 0; i < 12; i++){
//        test.emplace_back(in_points_array[i]);
//       gLogInfo << "in_points_array : " << in_points_array[i] << std::endl;
//    }
    ...
    *pfe_buffers_;
    ...
    for(int i = 0; i < out_boxes.size() / 7; i++){
        Box3D box;
        box.x = out_boxes[i * 7 + 0];
        box.y = out_boxes[i * 7 + 1];
        box.z = out_boxes[i * 7 + 2];
        box.l = out_boxes[i * 7 + 3];
        box.w = out_boxes[i * 7 + 4];
        box.h = out_boxes[i * 7 + 5];
        box.theta = out_boxes[i * 7 + 6];

        box.cls = out_labels[i];
